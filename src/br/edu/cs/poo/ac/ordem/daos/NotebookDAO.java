...
public class NotebookDAO extends DAOGenerico {
	...
	private String identificador(Notebook notebook) {
	...
	public boolean incluir(Notebook notebook) {
		if (buscar(identificador(notebook)) == null) {
			cadastroObjetos.incluir(notebook, identificador(notebook));
			return true;
		} else {
			return false;
		}
	...
	public boolean alterar(Notebook notebook) {
		if (buscar(identificador(notebook)) != null) {
			cadastroObjetos.alterar(notebook, identificador(notebook));
			return true;
		} else {
			return false;
		}
